...
function getReservationNotes(items, reservation) {
  const reservationData = reservation || {};
  const directNotes = String(
    reservationData.notes || reservationData.note || reservationData.reservationNotes || ""
  ).trim();
  ...
  for (const item of safeItems) {
    const metadata = item && item.metadata ? item.metadata : {};
    const notes = String(metadata.reservation_notes || "").trim();
    if (notes) {
      return notes;
    }
  }
...
async function sendCheckoutLeadEmail({ items, reservation, cartId, sessionId }) {
  ...
  const reservationNotes = getReservationNotes(items, reservation);
  const payload = {
